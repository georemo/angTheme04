...
export class GlobalControllerService {
  ...
  mainContent;
  footer;
  mainNavMenu: NodeListOf<Element>;
  testCss: NodeListOf<Element>;

  // Screen Size
  screenHeight: any;
  screenWidth: any;
  /*
    BOOTSTRAP SCREEN CATEGORIES
    xs (for phones - screens less than 768px wide)
    sm (for tablets - screens equal to or greater than 768px wide)
    md (for small laptops - screens equal to or greater than 992px wide)
    lg (for laptops and desktops - screens equal to or greater than 1200px wide)
  */
  BsScreenCategory; // Bootstrap Screen Category
  ...
  */
  ...
  initGlobals() {
    this.$body = $('body');
    this.inner = $('.main-wrapper > .inner');
    this.closedbar = $('.closedbar');
    this.body = $('body');
    this.sideLeft = $('#pageslide-left');
    ...
    this.footer = $('.main-wrapper > footer');
    this.mainNavMenu = document.querySelectorAll('.main-navigation-menu');
    this.testCss = document.querySelectorAll('.main-navigation-menu');
  }
  ...
  main() {
    ...
    this.detectMenuClick();
    $(document).ready(() => {
      console.log('starting GlobalControllerService::main()');
      var isIE8 = false,
        isIE9 = false,
      ...
      var Main = function () {
        ...
        var runNavigationMenu = function () {
          console.log('starting GlobalControllerService::runNavigationMenu()');
          if ($("body").hasClass("single-page") == false) {
            console.log('...001...$("body").hasClass("single-page") == false');
            $('.main-navigation-menu > li.active').addClass('open');
            console.log('...002...$("body").hasClass("single-page") == false');

            ///////////////////////////////
            // $('.main-navigation-menu').on('click', () => {
            //   $.each(this,(k,val) => {
            //     console.log(k);
            //   });
            // });

            // $('ul.main-navigation-menu').on('click', function () {
            //   console.log('click .ul.main-navigation-menu');
            //   console.log(document.querySelector('.main-navigation-menu > li a'));
            // });


            // const mainNavMenu = document.querySelectorAll('.main-navigation-menu');
            // mainNavMenu.forEach( (val, key, parent) => {
            //   console.log(val);
            //   console.log(key);
            // });
            // mainNavMenu.addEventListener("click", () => {
            //   console.log('click .main-navigation-menu > li a');
            // });
            // const mainNavMenu = document.querySelector('.main-navigation-menu > li a');
            // mainNavMenu.addEventListener("click", function (event) {
            //   console.log('click .main-navigation-menu');
            // });
            //////////////////////////////
              ...
              if ($windowWidth > 991) {
                ...
                if ($body.hasClass("sidebar-close")) {
                  ...
                  if ($body.hasClass("layout-boxed") || $body.hasClass("isMobile")) {
                    ...
                    closedbar.classList.remove("open"); //vanilla js. jq version substituted
                    closedbar[0].style.visibility = 'hidden';
                    closedbar[0].style.left = -closedbar[0].style.width;
                    // closedbar.css({
                    //   //translateZ: 0,
                    //   left: -closedbar.width()
                    // });
                  ...
                  if ($body.hasClass("layout-boxed") || $body.hasClass("isMobile")) {
                    ...
                    $(".main-container, footer .footer-inner, #horizontal-menu .container").velocity({
                      ...
                      configAnimation);
                    ///////////////////////////////////////////////////////////
                    closedbar[0].style.display = 'block';
                    activeAnimation = false;
                    closedbar[0].style.left = 0;
        ...
        const runClosedBarButton = function () {
          ...
          closedbar.addEventListener("mouseover", function (event) {
            ...
            if ($body.hasClass("layout-boxed") == false && $body.hasClass("isMobile") == false && closedbar.classList.contains('open')) {
              ...
              t = setTimeout(function () {
                closedbar[0].style.left = -closedbar[0].style.width;
                sideLeft[0].style.left = -sidebarWidth;
                sideLeft[0].style.zIndex = 1021;
                sideLeft[0].style.left = 0;
                /////
                closedbar.classList.remove("open"); //vanilla js. jq version substituted
                closedbar[0].style.visibility = 'hidden';
                closedbar[0].style.left = -closedbar[0].style.width;
          ...
          $('.save_style').on('click', function () {
            ...
            if ($body.hasClass('layout-boxed')) {
              ...
              $("body[class]").filter(function () {
                ...
                // include return null to harmonize typings
                return null;
              });
            } else {
        ...
        return {
          ...
          init: function () {
            ...
            const threshold = null;
            const execAsap = null;
            console.log('GlobalControllerService::Main::init 001');
            runWIndowResize(func, threshold, execAsap);
  ...
  initMenuDisplay() {
    const myElements = this.mainNavMenu;
    Array.from(myElements).forEach((val, index, arr_elements) => {
      console.log('Array.from(myElements).forEach()');
      console.log('val>>');
      console.log(val);
      console.log('index>>');
      console.log(index);
  ...
  detectMenuClick() { // start detectCss()
    this.mainNavMenu[0].addEventListener('click', (event) => {
      console.log('GlobalControllerService::detectMenuClick');
      const selMenu = this.getSelMenu(event);
      const menuID = this.getMenuID(selMenu);
      console.log('menuID>>');
      console.log(menuID);
      const $selMenu = $(`#${menuID}`);
      const isOpen = this.isOpenMenu(selMenu);
      console.log('isOpen>>');
      console.log(isOpen);
  ...
  getSelMenu(event): Element {
    let ret = null;
    if (event.target['tagName'] == 'SPAN') {
      console.log('if <span>');
      console.log(event.target['parentElement']['parentElement']);
      ret = event.target['parentElement']['parentElement'];
    }
    if (event.target['tagName'] == 'A') {
      console.log('if <a>');
      console.log(event.target['parentElement']);
      ret = event.target['parentElement'];
    }
    return ret;
  }

  // get sub-menu collection for selected menu-item
  getSubMenu(selMenu) {
    return selMenu.querySelectorAll('ul.sub-menu');
  }

  // check if selected menu has sub-menu
  hasSubMenu(selMenu) {
    let ret;
    if (this.getSubMenu(selMenu).length > 0) {
      ret = true;
    } else {
      ret = false;
    }
  ...
  getMenuID(selMenu) {
    const menuA = selMenu.querySelectorAll('a');
    const menuID = menuA[0].getAttribute('id');
    return menuID;
  }

  isOpenMenu(selMenu) {
    return selMenu.classList.contains('open');
  }

  openMenu(selMenu) {
    return selMenu.classList.add('open');
  }
